...
func getMinMaxXY(stoneStructures [][]Stone) (int, int, int, int) {
	...
	return minX - margin, minY, maxX + margin, maxY + margin
}
...
func main() {
	...
	minX, _, maxX, maxY := getMinMaxXY(stoneStructures)

	array := build2dArray(0, 0, maxX, maxY)
	array[0][500] = "+"
	for _, stoneStructure := range stoneStructures {
		var prevStone Stone
		for _, stone := range stoneStructure {
	...
	visualizeArray()
	var previousPath []Stone
	sandFellToAbyss := false
	for !sandFellToAbyss {
		previousPath = []Stone{}
		sand := Stone{x: 500, y: 0}
		sandMoving := true
		prevSand := sand
		for sandMoving {
			if sand.y >= maxY {
			...
			if sand == prevSand {
				...
				previousPath = append(previousPath, sand)
	...
	for _, stone := range previousPath {
		array[stone.y][stone.x] = "~"
	}
	visualizeArray()
}
